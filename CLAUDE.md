...
- `hw/ip/motion/rtl/` — Motion detection (axis_motion_detect, motion_core; axis_motion_detect_vibe, motion_core_vibe — selected per cfg_t.bg_model via generate gate in sparevideo_top)
...
- **bg_model selector** is a `cfg_t` field. `bg_model=BG_MODEL_EMA` (the default for every existing profile) keeps the EMA bg block; `bg_model=BG_MODEL_VIBE` swaps in the ViBe RTL block (`axis_motion_detect_vibe`, `motion_core_vibe`). Both paths are pixel-accurate vs their Python reference models at TOLERANCE=0. ViBe profiles run via `make run-pipeline CFG=default_vibe CTRL_FLOW=motion TOLERANCE=0` (full RTL sim). When `BG_MODEL_VIBE=1` is set at elaboration, a `generate` gate in `sparevideo_top` instantiates `axis_motion_detect_vibe` instead of `axis_motion_detect`.
...
**Compile-time RTL parameters are propagated from `CFG.<field>` at the top level.** Algorithm fields (e.g., `alpha_shift`, `alpha_shift_slow`, `grace_frames`, `gauss_en`, `morph_open_en`, `morph_close_en`, `morph_close_kernel`, `hflip_en`) are packed into a `cfg_t` struct and driven from the active profile selected by `CFG_NAME`. Inside `axis_motion_detect` they are still compile-time `-G` parameters; the top level unpacks the struct and passes each field as a named parameter. The config stamp in dv/sim/Makefile must include the `CFG` name so profile changes trigger recompilation.
...
**Mask cleanup via `axis_morph_clean`.** A combined 3×3 morphological opening (erode → dilate) followed by a parametrizable 3×3 or 5×5 closing (dilate → erode) sits between `axis_motion_detect` and the downstream mask consumers (CCL, overlay, mask display). The open removes single-pixel salt noise and features < 3 px wide; the close bridges small intra-object gaps so a single moving object produces one connected blob in CCL. Runtime gates: `morph_open_en` (set to `1` in `CFG_DEFAULT`) enables the open stage; `morph_close_en` (set to `1` in `CFG_DEFAULT`) enables the close stage; both `morph_open_en=0` and `morph_close_en=0` (e.g. profile `no_morph`) are zero-latency combinational bypasses. Compile-time `morph_close_kernel` (3 or 5) selects 3×3 or 5×5 closing. Consequence of open: thin features (far-field targets, 1-px lines) are erased — use the `thin_moving_line` synthetic source to exercise this. Python reference model composes `scipy.ndimage.grey_erosion`/`grey_dilation` with `mode='nearest'` when `morph_open_en=True`/`morph_close_en=True`, keeping RTL and model in agreement at `TOLERANCE=0` for both values. In motion.py/ccl_bbox.py, the EMA background update uses the **raw** (pre-morph) mask — matches the RTL where motion_detect drives EMA internally before morph is applied.
