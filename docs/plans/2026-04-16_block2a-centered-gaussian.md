...
2. `axis_fork_pipe`'s sideband pipeline grows to match (FF chain → **BRAM FIFO**)
3. `idx_pipe` in `axis_motion_detect` grows to match (register array → **BRAM FIFO**)
4. `axis_gauss3x3` adds an internal **output FIFO** (depth H_ACTIVE + 1) and **flush logic** for bottom/right borders
5. The Python model simplifies (standard centered convolution, no causal offset)
...
- **Sideband RGB**: delayed by H_ACTIVE + 3 cycles in `axis_fork_pipe` → the RGB data for pixel P arrives at the output stage at the same time as the Gaussian result for pixel P
- **idx_pipe**: delayed by H_ACTIVE + 3 stages → the memory address for pixel P is available at the same time
...
All three are in lockstep because the delays are identical. The existing pipeline algebra (`mem_rd_addr = idx_pipe[PIPE_STAGES-2]`, `mem_wr_addr = idx_pipe[PIPE_STAGES-1]`) remains valid — only the value of `PIPE_STAGES` changes.

### Resource cost (at 320×240)

| Resource | Causal (current) | Centered (proposed) | Delta |
|----------|-------------------|---------------------|-------|
| `axis_fork_pipe` sideband | 3 × 26 = 78 FFs | 323 × 26 bits = 1,050 bytes → **1 BRAM** | +1 BRAM, −78 FFs |
| `idx_pipe` | 3 × 17 = 51 FFs | 323 × 17 bits = 687 bytes → **1 BRAM** (shared) | +0–1 BRAM, −51 FFs |
| `axis_gauss3x3` output FIFO | — | 321 × 8 bits = 321 bytes → **1 BRAM** (shared) | +0–1 BRAM |
| `axis_gauss3x3` flush logic | — | ~30 FFs + mux | +30 FFs |
| **Total** | ~130 FFs | **2–3 BRAMs** + ~30 FFs | |

At 320px, all three FIFOs fit in 2 BRAM18K blocks (each is 18,432 bits = 2,304 bytes). This is a modest cost on any FPGA with >10 BRAMs.
...
### File: `hw/ip/axis/rtl/axis_fork_pipe.sv` (modify)

The current implementation is a pure FF shift chain. With `PIPE_STAGES = 323`, this becomes 323 × 26 = 8,398 FFs — wasteful and potentially timing-problematic.

**Change**: Add a `generate` block that selects the storage mechanism:

```systemverilog
localparam bit USE_BRAM = (PIPE_STAGES > 16);  // threshold: FF vs BRAM

generate
    if (USE_BRAM) begin : gen_bram_pipe
        // BRAM circular buffer for tdata, tlast, tuser, tvalid
        // Single BRAM: width = DATA_WIDTH + 3 (tlast + tuser + tvalid), depth = PIPE_STAGES
        // Write at wr_ptr, read at rd_ptr = wr_ptr - PIPE_STAGES (modular)
        // Gated by !pipe_stall_o
    end else begin : gen_ff_pipe
        // Existing FF shift chain (unchanged)
    end
endgenerate
```

The BRAM pipe stores `{tdata, tlast, tuser, tvalid}` packed into a single word. At 24+3 = 27 bits wide and 323 deep, this is 8,721 bits — fits comfortably in one BRAM18K (18,432 bits).

**Behavioral contract is unchanged**: the module still delays sidebands by `PIPE_STAGES` cycles, still exports `pipe_stall_o` and `beat_done_o`, still gates advancement on `!pipe_stall_o`. The fork acceptance tracking (a_accepted, b_accepted, both_done) is purely combinational and unaffected.

**BRAM read latency**: BRAM has 1-cycle read latency. The pipe output (`pipe_tdata_o`, etc.) comes from the BRAM read port, which is already 1 cycle behind the write — consistent with registered pipeline behavior. The `pipe_valid` signal (`tvalid_pipe[PIPE_STAGES-1]`) comes from the same BRAM read output.
...
- The fork pipeline continues advancing (draining sideband data) — `fork_stall` still governs this
...
This requires a small change to the fork's ready logic:

```systemverilog
logic gauss_busy;
assign gauss_busy = (GAUSS_EN != 0) ? u_gauss.busy_o : 1'b0;

// Modify: suppress input acceptance during Gaussian flush
assign s_axis_tready_o = !gauss_busy && (!pipe_valid || both_done);
```

Wait — `s_axis_tready_o` is driven by `axis_fork_pipe`. The flush suppression must be applied at the `axis_motion_detect` level, not inside the fork. Two options:

**Option A**: Override `s_axis_tready_o` at the `axis_motion_detect` output:
```systemverilog
assign s_axis_tready_o = u_fork.s_axis_tready_o && !gauss_busy;
```
This works because during flush, we don't want to accept new input from upstream.

**Option B**: Add `gauss_busy` as an input to `axis_fork_pipe` that suppresses acceptance.

Option A is simpler and doesn't change `axis_fork_pipe`'s interface.
...
| File | Reason |
|------|--------|
| `sparevideo_top.sv` | External interface of `axis_motion_detect` unchanged |
...
### Phase 2: RTL — `axis_fork_pipe` changes

- [ ] Add BRAM-backed circular buffer path (generate block, `USE_BRAM` when `PIPE_STAGES > 16`)
- [ ] Verify behavioral equivalence with FF chain for small depths (existing tests)
- [ ] `make lint` passes

### Phase 3: RTL — `axis_motion_detect` changes
