...
├── axis_scale2x       (u_scale2x)       — 2x spatial upscaler (NN or bilinear); compile-time gate (SCALER=1)
...
| `clk_pix_out_i` | input | 1 | Output pixel clock — VGA output domain. Tied to `clk_pix_in_i` when `SCALER=0`; runs at 4x `clk_pix_in_i` when `SCALER=1` to drain the 2x-upscaled stream. |
...
| `SCALER` | `int` | `0` | Compile-time gate for `u_scale2x`. `0` = output equals input dims (zero-latency bridge); `1` = `axis_scale2x` is instantiated and the VGA controller uses the `*_OUT_2X` localparams from `sparevideo_pkg`. |
| `SCALE_FILTER` | `string` | `"bilinear"` | Filter mode for `axis_scale2x` when `SCALER=1`: `"nn"` (nearest-neighbour pixel doubling) or `"bilinear"`. Ignored when `SCALER=0`. |
...
```
  ...
  │                         ▼                                                    │
  │                 ┌──────────────┐  2x upscaler when SCALER=1; flat bridge     │
  │                 │  u_scale2x   │  when SCALER=0 (no axis_scale2x instance)   │
  │                 └──────┬───────┘                                             │
  │                        │  scale2x_to_pix_out (axis_if: RGB + tlast + tuser)  │
  │                        ▼                                                     │
  │                 ┌──────────────┐  CDC: clk_dsp → clk_pix_out                 │
  │                 │  u_fifo_out  │  (DEPTH = 1024 when SCALER=1, else 256)     │
  │                 └──────┬───────┘                                             │
...
- **u_fifo_in / u_fifo_out**: CDC between the pixel domains (`clk_pix_in` / `clk_pix_out`) and `clk_dsp`; `IN_FIFO_DEPTH = 128`, `OUT_FIFO_DEPTH = 256` (bumps to `1024` when `SCALER=1` to absorb the scaler's burst output, since it emits up to 4 beats per input pixel). Overflow detected by SVA (§9).
- **u_scale2x**: 2x spatial upscaler instantiated under a generate gate when `SCALER=1`; bilinear or NN filter selected by `SCALE_FILTER`. When `SCALER=0`, the gate produces a zero-latency combinational bridge (`assign` chain from `gamma_to_pix_out` to `scale2x_to_pix_out`) so the path is byte-identical to pre-scaler runs. See [axis_scale2x-arch.md](axis_scale2x-arch.md).
...
| `clk_pix_out` | `SCALER=0`: tied to `clk_pix_in`. `SCALER=1`: 4x `clk_pix_in` so VGA can drain the 2x-upscaled output. | `u_fifo_out` read side, `u_vga`, VGA reset gating |
| `clk_dsp` | 100 MHz | `u_fifo_in` read side, `u_fork`, `u_motion_detect`, `u_ram`, `u_morph_open`, `u_ccl`, `u_overlay_bbox`, `u_gamma_cor`, `u_scale2x` (when `SCALER=1`), `u_fifo_out` write side |
