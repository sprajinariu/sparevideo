...
In input-image terms: with `GAUSS_EN=0` opening deletes features narrower than 3 px; with `GAUSS_EN=1` only single isolated pixels are deleted, since the Gauss widens 1- and 2-px features above the morphological threshold. For salt-noise suppression (the primary design intent), `GAUSS_EN=1` is fully compatible — isolated-pixel speckles still produce single-pixel mask hits and are still erased by opening.
...
Each sub-module instantiates one `axis_window3x3<DATA_WIDTH=1, EDGE_REPLICATE>`, applies a combinational 9-input reduction across `window_o[8:0]` (AND for erode, OR for dilate), and registers the result. `tlast`/`tuser` are regenerated from a local `(out_col, out_row)` counter that advances on each `window_valid && !stall` beat: `tuser` at `(0, 0)`, `tlast` at `out_col == H_ACTIVE-1`. `stall = !m_axis_tready_i` freezes every register including the inner window primitive. `s_axis_tready_o` deasserts only during phantom cycles (`busy_o` from `axis_window3x3`); otherwise it follows `!stall`.

### 5.3 `axis_morph3x3_open` composite

A thin wrapper: instantiates `u_erode` and `u_dilate`, wires the internal AXIS link `mid_*` between them, and routes `enable_i` to both. `u_erode` sources the composite's `s_axis_tready_o`; `u_dilate` drives the composite's `m_axis_*` outputs. No combinational logic or state is added at this level.

### 5.4 `enable_i` bypass semantics

When `enable_i = 0`, each sub-module's combinational reduction is muxed out and `s_axis_*` is forwarded directly to `m_axis_*` (zero additional latency). The line buffers inside `axis_window3x3` are still instantiated but their output is unused. `enable_i` must be held stable across a complete frame; toggling mid-frame leaves the line buffers and output counter in inconsistent state.
