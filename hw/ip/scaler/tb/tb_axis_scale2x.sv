// Unit testbench for axis_scale2x (bilinear).
//
// Pattern inventory:
//   t1: 2x2 ramp; verifies top-edge + right-edge replicate and the avg2 LSB.
//   t2: 2x2 ramp + asymmetric stall (downstream tready toggled per beat);
//       verifies the FSM holds the input while emitting both output rows.

`timescale 1ns / 1ps

module tb_axis_scale2x;

    parameter int H_ACTIVE_IN = 2;
    parameter int V_ACTIVE_IN = 2;
    ...
    initial begin : main
        ...
        //                                            0x000030, 0x000040}.
        // Bilinear: 16 output beats with averaging (golden hand-checked
        // in test_scale2x.py).
        for (r = 0; r < 2; r++) begin
            for (c = 0; c < 2; c++) begin
