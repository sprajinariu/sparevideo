...
// 3x3 morphological dilation on a 1-bit mask stream, implemented as a thin
// wrapper over axis_window3x3 (DATA_WIDTH=1, EDGE_REPLICATE).
//
// Reduction: output = OR of all 9 taps of the 3x3 window (foreground when
// the pixel or any neighbour is 1). Multiplications and adds are none --
// pure gate tree.
//
...
//
// enable_i: when 1, the window-based dilate path drives the output. When 0,
...
module axis_morph3x3_dilate #(
    ...
    // ---- 9-way OR reduction (combinational dilate) ----
    logic dilate_bit;
    always_comb begin
        dilate_bit = window[0] | window[1] | window[2]
