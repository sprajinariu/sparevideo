                    ...
                    while (captured_out < V * H) begin
                        ...
                        if (m_tvalid && dut_tready) begin
                            if (captured_out == 0) begin
                                if (!m_tuser) begin
                                    $display("FAIL test9: tuser not asserted on first output pixel");
                                    num_errors = num_errors + 1;
                                end
                            end else begin
                                if (m_tuser) begin
                                    $display("FAIL test9: tuser asserted on non-first pixel %0d", captured_out);
                                    num_errors = num_errors + 1;
                                end
                            end
                            ...
                            if (m_tlast && ((captured_out % H) != (H - 1))) begin
                                $display("FAIL test9: tlast asserted at mid-row (idx %0d)", captured_out);
                                num_errors = num_errors + 1;
                            end
                            if (!m_tlast && ((captured_out % H) == (H - 1))) begin
                                $display("FAIL test9: tlast NOT asserted at row end (idx %0d)", captured_out);
                                num_errors = num_errors + 1;
                            end
